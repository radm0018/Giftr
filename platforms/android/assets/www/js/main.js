...
let currentPerson;
//localStorage.removeItem("radm0018");
...
function saveToLocalStorage(){
    localStorage.setItem("radm0018", JSON.stringify(people));
}

function getFromLocalStorage(){
    if(!localStorage.getItem("radm0018")){
        console.log("No data found");
    }
    else{
        people = JSON.parse(localStorage.getItem("radm0018"));
...
function displayGiftList(){
    ...
    for(let i = 0; i < length; i++){
        ...
        liGift.classList = "table-view-cell media";
        liGift.setAttribute("dataId", ideaList[i].id);
        spanDelete = document.createElement("span");
        spanDelete.classList = "pull-right icon icon-trash midline";
        divIdeaDisplay = document.createElement("div");
        divIdeaDisplay.textContent = ideaList[i].idea;
        divIdeaDisplay.classList = "media-body";
